...
webcam = cv2.VideoCapture("/home/fdiao/Videos/test/sun0/DMS_20221024_sun0_wyt.mp4")
...
face_cascade = cv2.CascadeClassifier(os.path.join(
    dirname, 'lbpcascade_frontalface_improved.xml'))
landmarks_detector = dlib.shape_predictor(os.path.join(
    dirname, 'shape_predictor_5_face_landmarks.dat'))
...
def main():
    ...
    # write to file
    video_writer = cv2.VideoWriter('eyetracking_customer.avi', cv2.VideoWriter_fourcc(
        *'MJPG'), 30, (frame_width, frame_height))
    ...
    while True:
        ...
        if landmarks is not None:
            ...
            right_eyes = list(
                filter(lambda x: not x.eye_sample.is_left, eye_preds))

            if left_eyes:
                left_eye = smooth_eye_landmarks(
                    left_eyes[0], left_eye, smoothing=0.1)
            if right_eyes:
            ...
            for ep in [left_eye, right_eye]:
                ...
                if ep.eye_sample.is_left:
                    gaze[1] = -gaze[1]
                    # print("Left eye",ep.landmarks[-2])
                    print("Left eye Pitch", gaze[0]*180/np.pi)
                    print("Left eye Yaw", gaze[1]*180/np.pi)
                    yaw_list.append(gaze[1]*180/np.pi)
                    # CV TEXT
                    cv2.putText(orig_frame, "Left eye Yaw: " + str(round(
                        gaze[0]*180/np.pi, 4)), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

                else:
                    # print("Right eye",ep.landmarks[-2])
                    print("Right eye Pitch", gaze[0]*180/np.pi)
                    print("Right eye Yaw", gaze[1]*180/np.pi)
                    yaw_list.append(gaze[1]*180/np.pi)
                    # CV TEXT
                    cv2.putText(orig_frame, "Right eye Yaw: " + str(round(
                        gaze[0]*180/np.pi, 4)), (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

                util.gaze.draw_gaze(
                    orig_frame, ep.landmarks[-2], gaze, length=30.0, thickness=2)
            left_right_avg_yaw = (np.mean(yaw_list))
            print("Average Yaw", left_right_avg_yaw)
            eye_avg_buffer.append(abs(left_right_avg_yaw))
            if len(eye_avg_buffer) > eye_buffer_size:
                eye_avg_buffer.pop(0)
                ave_yaw = np.sum(
                    eye_avg_buffer, axis=0) / eye_buffer_size

                cv2.putText(orig_frame, "Average Yaw: " + str(round(left_right_avg_yaw, 4)),
                            (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                if ave_yaw > 20 and left_right_avg_yaw < 0:
                    cv2.putText(orig_frame, "Looking Left", (10, 150),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                elif ave_yaw > 15 and left_right_avg_yaw > 0:
                    cv2.putText(orig_frame, "Looking Right", (10, 150),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

        cv2.imshow("demo", orig_frame)
        cv2.waitKey(1)
...
def segment_eyes(frame, landmarks, ow=160, oh=96):
    ...
    for corner1, corner2, is_left in [(2, 3, True), (0, 1, False)]:
        ...
        inv_transform_mat = (inv_translate_mat *
                             inv_scale_mat * inv_center_mat)
...
def smooth_eye_landmarks(eye: EyePrediction, prev_eye: Optional[EyePrediction], smoothing=0.2, gaze_smoothing=0.4):
    ...
    return EyePrediction(
        ...
        landmarks=smoothing * prev_eye.landmarks +
        (1 - smoothing) * eye.landmarks,
    ...
    for eye in eyes:
        with torch.no_grad():
            ...
            landmarks = np.asarray(
                np.matmul(landmarks, eye.transform_inv.T))[:, :2]
            assert landmarks.shape == (34, 2)
            result.append(EyePrediction(eye_sample=eye,
                          landmarks=landmarks, gaze=gaze))
    return result
